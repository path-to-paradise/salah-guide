...
The site builds to a static `dist/` folder (`npm run build`) and can be hosted anywhere that serves static files with SPA-style routing fallback (unmatched routes → `index.html`) — the production deployment runs on **Cloudflare Workers** (static assets), currently live at [salah-guide.path-to-paradise.workers.dev](https://salah-guide.path-to-paradise.workers.dev). Remember to set `VITE_GA_MEASUREMENT_ID` as a **build-time** environment variable in the Cloudflare dashboard for this Worker (not just locally) if you want analytics on the deployed site — Vite inlines it during `npm run build`, so it has no effect if only set at runtime.

There's an unmerged `cloudflare/workers-autoconfig` branch (auto-generated by Cloudflare's GitHub integration) that adds an in-repo `wrangler.jsonc` plus `@cloudflare/vite-plugin`, for anyone who wants to run `wrangler dev` / `wrangler deploy` locally instead of relying on the dashboard's git-connected build. It predates several commits on `main`, so merge with care.
